...
What are the differences between Rawhide and ELN?::
    ...
    there are some xref:buildroot.adoc[buildroot differences]
    in xref:buildroot.adoc#_compiler_flags_and_other_tweaks[compiler flags]
    and xref:buildroot.adoc#_distribution_related_macro_definitions[global macros]
    that can result in build differences.

[NOTE]
====
Many packages have ``0%{?fedora}`` and/or ``0%{?rhel}`` conditionals in their specfiles that produce
