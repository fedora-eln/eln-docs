...
** Set the `configuration.control.pause` value in https://github.com/fedora-eln/elnbuildsync-config/blob/production/elnbuildsync_dynamic.yaml[ELNBuildSync dynamic configuration] to `true`
. Wait for the Fedora ELN Buildroot to regenerate

 $ koji wait-repo eln-build --request

. Update the https://src.fedoraproject.org/rpms/fedora-eln-release[fedora-eln-release] package
.. Set `fedora_dist_version` to the next major release version.
.. Rebuild the `fedora-eln-release` package.

. Rebuild the post-branch changes to the `fedora-repos` package:

 $ git checkout rawhide && git pull && fedpkg --release eln build

. Update `configuration.control.trigger_tag` in https://github.com/fedora-eln/elnbuildsync-config/blob/production/elnbuildsync_dynamic.yaml[ELNBuildSync dynamic configuration] to the next Fedora tag (`f42`)

. Resume ELN package rebuilds in ELNBuildSync
** Set the `configuration.control.pause` value in https://github.com/fedora-eln/elnbuildsync-config/blob/production/elnbuildsync_dynamic.yaml[ELNBuildSync dynamic configuration] to `false`
...
.. Rebuild the `fedora-eln-release` package.
. Rebuild the post-branch changes to the `fedora-repos` package.
. Update `kiwibuild_version` in https://forge.fedoraproject.org/releng/pungi-fedora/src/branch/eln/fedora/override.conf[pungi configuration] to the same value as `rpm.macro.rhel` above
. Update `configuration.control.trigger_tag` in https://github.com/fedora-eln/elnbuildsync-config/blob/production/elnbuildsync_dynamic.yaml[ELNBuildSync dynamic configuration] to the next Fedora tag (`f41`)
. Resume ELN package rebuilds in ELNBuildSync
** Set the `configuration.control.pause` value in https://github.com/fedora-eln/elnbuildsync-config/blob/production/elnbuildsync_dynamic.yaml[ELNBuildSync dynamic configuration] to `false`
