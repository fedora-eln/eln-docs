...
https://elnbuildsync.fedoraproject.org/status.html[the
ELN Rebuild Status Page].  ELN Extras packages get rebuilt much more
frequently than in EPEL.
...
https://github.com/fedora-eln/content-resolver/blob/master/config_specs/workload.yaml[example workload yaml file].
...
* the SRPM name has to be added to the ``configuration.control.exclude`` list in https://github.com/fedora-eln/elnbuildsync-config/blob/production/elnbuildsync_dynamic.yaml[ELNBuildSync dynamic configuration] to avoid accidental rebuilds
* the needed subpackages need to be added to Content Resolver for ELN Extras as usual (https://github.com/fedora-eln/content-resolver-input/pull/1124[example])
